<?php namespace CodeIgniterCart {
    ...
    class Cart
    {
        ...
        public function insert($items = []): bool
        {
            ...
            if ( isset($items[ 'id_item' ]) ) {
                ...
                foreach ( $items as $val ) {
                    if ( is_array($val) && isset($val[ 'id_item' ]) && $this->_insert($val) ) {
                        $save_cart = true;
                    }
                }
            }
        ...
        protected function _insert($items = [])
        {
            ...
            if ( ! isset($items[ 'id_item' ], $items[ 'jumlah_item' ], $items[ 'harga_item' ], $items[ 'nama_item' ]) ) {
            ...
            $items[ 'jumlah_item' ] = (float)$items[ 'jumlah_item' ];
            ...
            if ( ! preg_match('/^[' . $this->productIdRules . ']+$/i', $items[ 'id_item' ]) ) {
            ...
            if ( $this->productNameSafe && ! preg_match('/^[' . $this->productNameRules . ']+$/i' . ( true ? 'u' : '' ), $items[ 'nama_item' ]) ) {
                log_message('error', 'An invalid name was submitted as the product name: ' . $items[ 'nama_item' ] . ' The name can only contain alpha-numeric characters, dashes, underscores, colons, and spaces');
                return false;
            }
            ...
            $items[ 'harga_item' ] = (float)$items[ 'harga_item' ];
            ...
            $old_quantity = isset($this->cartContents[ $rowid ][ 'jumlah_item' ]) ? (int)$this->cartContents[ $rowid ][ 'jumlah_item' ] : 0;

            // Re-create the entry, just to make sure our index contains only the data from this submission
            $items[ 'rowid' ] = $rowid;
            $items[ 'jumlah_item' ] += $old_quantity;
        ...
        protected function _update($items = []): bool
        {
            ...
            if ( isset($items[ 'harga_item' ]) ) {
                $items[ 'harga_item' ] = (float)$items[ 'harga_item' ];
            }

            // product id & name shouldn't be changed
            foreach ( array_diff($keys, [ 'id_item', 'nama_item' ]) as $key ) {
        ...
        protected function saveCart(): bool
        {
            ...
            foreach ( $this->cartContents as $key => $val ) {
                ...
                if ( ! is_array($val) || ! isset($val[ 'harga_item' ], $val[ 'jumlah_item' ]) ) {
                    continue;
                }

                $this->cartContents[ 'cart_total' ] += ( $val[ 'harga_item' ] * $val[ 'jumlah_item' ] );
                $this->cartContents[ 'total_items' ] += $val[ 'jumlah_item' ];
                $this->cartContents[ $key ][ 'subtotal' ] = ( $this->cartContents[ $key ][ 'harga_item' ] * $this->cartContents[ $key ][ 'jumlah_item' ] );
            }
